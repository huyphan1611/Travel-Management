...
public class ViewAdCombo extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    ViewAdCombo frame = new ViewAdCombo("");
    ...
    public ViewAdCombo(String username) throws SQLException {
        //conn = Javaconnect.getDBConnection();
        //setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        //setBackground(new Color(255,255,204));
        setBounds(500, 200, 950, 680);
        ...
        if (username.equals("admin")==true) {
            ...
            try {
                Conn c = new Conn();
                String displayCustomersql = "select * from bookcombo";
            ...
            try {
                Conn c = new Conn();
                String displayCustomersql = "select * from bookcombo where username = '"+username+"'";
                ResultSet rs = c.s.executeQuery(displayCustomersql);
                table.setModel(DbUtils.resultSetToTableModel(rs));
            } catch (Exception e1) {
                e1.printStackTrace();
        ...
        lbf = new JLabel("Buffet");
        lbf.setBounds(420, 15, 76, 14);
        contentPane.add(lbf);*/


        lblId = new JLabel("ID Type");
        lblId.setBounds(410, 15, 90, 14);
        contentPane.add(lblId);

        JLabel l3 = new JLabel("ID Number");
        l3.setBounds(540, 15, 90, 14);
        contentPane.add(l3);

        JLabel l4 = new JLabel("Phone");
        l4.setBounds(670, 15, 90, 14);
        contentPane.add(l4);
